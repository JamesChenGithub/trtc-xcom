...
namespace xcom
{
    ...
    xcom_log_crypt::xcom_log_crypt():is_crypt_(false) {
    }
    ...
    uint32_t xcom_log_crypt::getHeaderLen() {
        return sizeof(char) + sizeof(uint16_t) + sizeof(uint16_t);
    }
    
    uint32_t xcom_log_crypt::getTailerLen() {
        return sizeof(tMagicEnd);
    }
    
    
    uint16_t xcom_log_crypt::getLogLength(const char *const _data, int32_t _len) {
    ...
    uint32_t xcom_log_crypt::getValidLength(const char *const _data,int32_t _len){
        if (_len < getHeaderLen()){
            return 0;
        }
    ...
    uint16_t xcom_log_crypt::getSeq(bool _sync) {
    ...
    void xcom_log_crypt::cryptSyncLog(const char *const _log_data, int32_t _input_len,
                                 xcom_auto_buffer &_out_buff) {
    ...
    void xcom_log_crypt::cryptAsyncLog(const char *const _log_data, int32_t _input_len,
                                  xcom_auto_buffer &_out_buff) {
        if(_input_len < 1){
            return;
        }
