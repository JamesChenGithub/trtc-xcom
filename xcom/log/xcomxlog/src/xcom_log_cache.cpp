...
namespace xcom
{
    ...
    xcom_log_cache::xcom_log_cache(bool bOwner)
    : _bOwner(bOwner), _pAddr(NULL), _iLength(0), _bCreate(false) {
    }
    
    xcom_log_cache::~xcom_log_cache() {
        if (_bOwner) {
            munmap();
        }
    }
    
    void xcom_log_cache::mmap(size_t length, int prot, int flags, int fd, off_t offset) {
        if (_bOwner) {
            munmap();
        }
    ...
    void xcom_log_cache::mmap(const char *file, size_t length) {
        if(length <= 0){
            return;
        }
    ...
    void xcom_log_cache::msync(bool bSync) {
        int ret = 0;
#ifdef _WIN32
